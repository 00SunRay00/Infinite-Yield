...
addcmd('rvanka', {}, function(args, speaker)
    local target = getPlayer(args[1], speaker)[1] -- Получаем первого игрока из аргумента <ник>
    if target then
        -- Выполняем orbit <nick> 10 1
        execCmd('orbit ' .. target .. ' 10 1', speaker)
        -- Выполняем spin 10000
        execCmd('spin 10000', speaker)
        notify('Rvanka', 'Started rvanka for ' .. target)
    else
